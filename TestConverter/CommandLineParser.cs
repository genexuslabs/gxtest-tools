...
namespace GeneXus.GXtest.Tools.TestConverter.Helpers
{
    public class CommandLineParser
	{
		...
		public string GetUsage(string err)
		{
			...
			maxTabPos += 2;
			string argDscLineFormat = string.Format("{{0,-{0}:S}}{{1}}", maxTabPos);
			...
			if (allowArgFile)
			{
				shortUsage.AppendFormat(CultureInfo.InvariantCulture, "[@argfile]");
				//TODO format line using FormatSingleLineMaxChars
				longUsage.AppendFormat(CultureInfo.InvariantCulture, argDscLineFormat, "@argfile", "Read arguments from a file.").Append(Environment.NewLine);
			}
			...
			foreach (MemberInfoUsage member in GetMembers())
			{
				...
				if (null != m)
				{
					...
					{
						longUsage.Append(Environment.NewLine).AppendFormat(CultureInfo.InvariantCulture, "{0,-" + maxTabPos + ":S}- {1} -", "", member.usage.Category).Append(Environment.NewLine);
						lastCategory = member.usage.Category;
					}

					// Usage
					longUsage.AppendFormat(CultureInfo.InvariantCulture, argDscLineFormat, m.Groups["usage"].Value, string.Empty);
					...
					for (int g = 1; g < formattedLines.Length; ++g)
						longUsage.AppendFormat(CultureInfo.InvariantCulture, argDscLineFormat, string.Empty, formattedLines[g]).Append(Environment.NewLine);
				}
			}
			...
			for (int g = 1; g < shortLines.Length; ++g)
				shortUsage.AppendFormat(CultureInfo.InvariantCulture, "{0,-7:S}{1}", "", shortLines[g]).Append(Environment.NewLine);
			...
			if ((null != parser) && ("" != parser.EnvironmentDefaults))
			{
				...
				usage.AppendFormat(CultureInfo.InvariantCulture, "Switches may be preset in the {0} environment variable.", parser.EnvironmentDefaults).Append(Environment.NewLine);
